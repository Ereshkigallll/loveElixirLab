# synthesis/intermediate.py
# 中级物品合成逻辑，支持一级和中级物品，恢复固定比例失败机制

from data import intermediate_recipes, initial_items, item_entries, easter_eggs
...
def synthesize_intermediate(inventory, selected_items):
    ...
    if len(selected_items) == 1:
        return None, None, None, None, "中级合成需至少两种物品！"

    # 检查输入物品等级
    if any(item['item_type'] == 'third' for item in selected_items):
        return None, None, None, None, "中级合成不可使用三级物品！"
    ...
    if random.random() < FAILURE_PROBABILITY:
        ...
        # 彩蛋物品（从 easter_eggs 选择初级或中级）
        easter_egg = None
        if random.random() < EASTER_EGG_PROBABILITY:
            valid_eggs = [egg for egg in easter_eggs if egg['item_type'] in ['initial', 'intermediate']]
            if valid_eggs:
        ...
        return None, new_inventory, returned_items, easter_egg, "中级合成失败！"
    ...
    return result, new_inventory, [], byproduct, f"中级物品合成成功！获得：{item_name}" + (
